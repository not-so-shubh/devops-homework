...
if [[ "$MISSING" -eq 0 ]]; then pass 'required 1–11 repository structure exists'; else fail 'required repository files are missing'; fi
...
if "$ROOT_DIR/01-linux-fundamentals/link-practice.sh" > "$EVIDENCE_DIR/linux-link-demo.txt" 2>&1; then
  pass 'Linux link demonstration'
else
  fail 'Linux link demonstration'
fi

if printf 'verification-output\n' | SYSTEM_INFO_BASE_DIR="$TMP_DIR" "$ROOT_DIR/02-shell-scripting/system-info.sh" > "$EVIDENCE_DIR/shell-script-output.txt" 2>&1; then
  pass 'shell system-information script'
else
  fail 'shell system-information script'
fi

if NETWORK_EXTERNAL=0 "$ROOT_DIR/03-networking-fundamentals/collect-network-info.sh" > "$EVIDENCE_DIR/network-info.txt" 2>&1; then
  pass 'network information collector'
else
  fail 'network information collector'
fi

if "$ROOT_DIR/04-git-github/git-practice-demo.sh" > "$EVIDENCE_DIR/git-practice-output.txt" 2>&1; then
  pass 'Git commit/cherry-pick demonstration'
else
  fail 'Git commit/cherry-pick demonstration'
fi
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  blocked 'Docker Compose CLI unavailable; runtime/config checks skipped'
fi
...
if command -v kubectl >/dev/null 2>&1; then
  kubectl version --client >/dev/null 2>&1 && pass 'kubectl client is available' || fail 'kubectl client check failed'
else
  blocked 'kubectl is not installed'
fi
if command -v minikube >/dev/null 2>&1; then
  minikube version >/dev/null 2>&1 && pass 'Minikube is available' || fail 'Minikube version check failed'
else
  blocked 'Minikube is not installed'
fi
